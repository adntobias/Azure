...
export class TeamsWebhooksOutgoingWebhook implements IOutgoingWebhook {
  ...
  public requestHandler(
    ...
    if (securityToken && securityToken.length > 0) {
      ...
      if (msgHash === auth) {
        ...
        const scrubbedText = TeamsWebhooksOutgoingWebhook.scrubMessage(
          incoming.text
        );
        message =
          TeamsWebhooksOutgoingWebhook.processAuthenticatedRequest(
            scrubbedText
          );
      } else {
  ...
  private static processAuthenticatedRequest(
    incomingText: string
    ...
    const selectedPlanet: any = planets.filter(
      (planet) =>
        (planet.name as string).trim().toLowerCase() ===
    ...
    if (!selectedPlanet || !selectedPlanet.length) {
      ...
      const adaptiveCard = TeamsWebhooksOutgoingWebhook.getPlanetDetailCard(
        selectedPlanet[0]
      );
      message.type = "result";
